...
portfolio_names <- names(NewData)
portfolio_names <- portfolio_names[c(6:30)]
...
FFparameters <- data.frame("portfolio"=portfolio_names, "alpha"=storage.ffalpha, "mktfactorbeta"=storage.ffbeta, "sizefactorbeta"=storage.ffbetasize, "valuefactorbeta"=storage.ffbetaBM)
FFparameters
...
#======================================================================
# calculate expected returns as per FF model
#======================================================================
...
mat1 <- matrix(,nrow=24683,ncol=25) # create a blank matrix of 25 columns, one for each portfolio


for (j in 1:25) # for each portfolio i.e. each column
{
  for (i in 1:24683) # for each date, i.e. each row
  {
    mat1[i,j] <- FFparameters$alpha[j] + FFparameters$mktfactorbeta[j]*NewData$Mkt.RF[i] + FFparameters$sizefactorbeta[j]*NewData$SMB[i] + FFparameters$valuefactorbeta[j]*NewData$HML[i]
    # calculation of expected returns
  }
}  

expectedreturns <- data.frame(mat1) # convert the oputput expected returns to a data frame

names(expectedreturns) <- portfolio_names # rename columns to portfolio names

expectedreturns 

expectedreturns[1] # 1st column i.e. exp returns of 1st portfolios

expectedreturns <- as.numeric(unlist(expectedreturns)) # coverting type from list to numeric

mean(expectedreturns[1])

#===========================================================================
#Average Returns (actual vs expected)
#==========================================================================
# calculation of average expected returns
#------------------------------------------------
avg_exp_returns <- vector()
for (i in 1:25){
  avg_exp_returns[i] = mean(expectedreturns[i])
...
actualreturns <- NewData[,c(6:30)]
typeof(actualreturns)
actualreturns <- as.numeric(unlist(actualreturns))

#calculation of average actual returns
#-----------------------------------------------
avg_act_returns <- vector()
for (i in 1:25){
  avg_act_returns[i] = mean(actualreturns[i])
  
}

# plot actual returns vs expected returns
#-----------------------------------------
plot(avg_act_returns,avg_exp_returns)
#==========================================================================

NewData$Date <- as.character(unlist(NewData$Date))
NewData$Date1 <- as.Date(NewData$Date,"%Y%m%d")

NewData$year <- as.numeric(format(NewData$Date1,"%Y"))

max(NewData$year) # 2020
min(NewData$year) # 1926

Beg <- 1926
End <- 2020
noofyears = 95

c <- list() # list of matrices with each matrix for each portfolio for annual betas

matptf1 <- matrix(,95,4) # create a matrix for 1st protfolio having alpha and betas for each year for 95 years
